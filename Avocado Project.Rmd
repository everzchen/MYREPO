...
We predict that avocado type is a predictor for its average price. It's likely that conventional avocados are not priced as high as the organic avocados. We will calculate the average price of avocados for each avocado type. To do this, we will create a new dataset, called avocado_type. Using the group_by command and the summarize command, we are going to create a new variable, 'averageprice_type' set equal to the mean average price of avocados for each of the two types. 

```{r}
avocado_type<-avocado%>%
  group_by(type)%>%
  summarize(averageprice_type=mean(AveragePrice))

view(avocado_type)
```

As anticipated, the average price of conventional avocados are cheaper, 1.16, is cheaper than the average price of organic avocados, 1.64. 





To examine if the relationship between avocado type and average price is statistically significant, we want to run a linear regression model. The outcome variable, 'averageprice' has a nice symmetric distribution. It looks approximately normal, which meets the necessary assumptions in interpreting the results. 


```{r}
#Model 1: simple bivariate regression, linear model of average price (dep) as a function of avocado type (pred), data set=avocado
mod1<-lm(AveragePrice~type,data=avocado) #outcome on left, predictor on right 

summary(mod1)
```
The coefficient of the linear regression model is statistically significant. We can reject the null hypothesis that the coefficient is zero and conclude that there is a relationship between avocado type and its average price. 

residual standard error = RMSE, 0.317. We are off by $0.317 dollars on average.
