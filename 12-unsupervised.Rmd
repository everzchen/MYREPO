...
Supervised learning: we train models to create preditions, then compare their predictions on out of sample answers to the right answer. We can (and do) iteratively improve these models. The computer is trained to come up with predictions that are as close as opposible to the right answer. Example - conditional means, regression models

Unsupervised learning: an algorithm comes up with its own classification based on patterns in the data. There aren't really outcomes to predict in unsupervised learning. Instead, we're allowing the algorithm to generate insights into our data. There's not really a dependent variable. We're letting the algorithm sift through the data and give us some insights, what patterns it sees, what cases are close or further away from one another.]

Two types of unsupervised learning: 

Association - generating a series of 'rules' based on how people behave. Recommendation engines work this way. If a customer buys a dog bowl, dog bed, and a leash - then it will recommend the customer to also buy a collar. Same with retail locations - structured around association rules (bread, peanut butter, jelly)

Clustering - covers problems where we want to classify groups in the data. Different from classification beause we don't know a priori what the groups are.
...
The SFO survey data is pretty standard survey data, the kind you might get back from surveymonkey or qualtrics or other similar survey software. It surveys the passengers that visited SFO on a particular day.
...
k=2:7 (two to 7 clusters)
nrep=20 (do this 20 different times)
...
C1 and C2 gives us very different cluster sizes. What's going on? This algorithm is sensitive if the group means aren't pretty clear.The data isn't strongly separated by these different groups, so we're going to have to try and find a way to get a real stable result out of this particular data. To start, run this command where we're going to bootstrap that flex clustering that we were doing before.
...
To visualize this, we'll use the `bootFlexclust` command, which will run the clustering algorithm on a subset of the data a certain number of times. This is called bootstrapping, and it shares many properties in common with cross-fold validation from last time. I'm going to do this for four different numbers of groups, from 2 through 5, then visualize the results. (We will repeat this analysis 50 times)
...
What do the different groups indicate about the individuals in them? Do they inhabit any recognizable patterns?
...
Now we're getting somehwere: One group is overwhelmingly composed of business travelers, and is much more likely to include frequent or moderate travelers. 
...
'-cluster' = drop the cluster assignment
```{r}
...
dependent variable = did you have a problem while visiting SFO? how likely are you to say that your experience was outstanding?
...
pred1
```
Group 1 had a 13.6% probability of having a problem. Group 2 had a 13.4% chance of having a problem. Group 3 and 4 had a 10% chance of having a problem.
...
pred_data
```
When compared with the business travelers (group1), group 2 were statistically more likely to report that their experience was outstanding.
