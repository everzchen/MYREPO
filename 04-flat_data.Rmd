...
Flat data is data that is arranged with one case per row, with one column per variable-- more or less. Rows may or may not be labeled. Each column is a variable, a characteristic of that person/organization. Columns are almost always labeled. It's stored in a variety of formats, with different conventions. Our goal is to get it into the most useful format for analysis: what's known as tidy data. 

Types of flat data files: delimited (comma-separated, csv; tab-separated, tsv), Fixed width (fwf, specify where in the file you'll find a variable, from point 2 to point 4, this is where you're going to find this variable).

Other statistical languages: Stata, SPSS, SAS

Data wrangling = Data munging, the process of converting raw data into a format that can then be used for data analysis and processing.

Keys for organizing flat data: 
1. No extraneous information (metadata) included inside dataset
2. Clear column (variable) names
3. Clear value labels
4. Understanding units of analysis
...
#Write the hsb dataset as a file. The write_csv command assumes its a csv file. We specify its path when the file is saved.
write_csv(hsb,path="hsb.csv")

#Check it out
head(hsb)

##Need these for later, list of names for the dataset.
my.names<-names(hsb)

#Write this in a variety of formats to be used later
#tab-separated variable, "\t" = tab delimiter
...
## store the width settings as 'my_positions' using the command, 'fwf_widths'            
my_positions<-fwf_widths(my.widths)

#read fixed-width file, telling the program that the column widths are stored as my_positions
...
### Stata
Stata is a very popular statistical programming language among economists and social scientists. Stata files are stored in `dta` format. We will use the Haven package, which is designed to read in a bunch of data form different statistical programming languages.
...
## What are databases?
A database contains...
1. multiple tables - it has more than one flat file within it
2. certain tables are linked to one another via certain IDs
3. can contain many different units of analysis
4. can cover multiple domains. 

A dataset..
1. have unique IDs for each case
2. have single units of analysis
3. cover a single conceptual domain

A dataset is NOT a database.

Database can be used to generate single flat files. Each flat file can be a dataset. 
...
## If this file does not exist(FALSE), go ahead and download it from the web and name it as 'free.xls'
...
## Read that file into memory as an R file, skip rows 1-4. Read in data starting on row 5. Don't import column names.
...
#Get rid of unwanted rows, get rid of every row that has missing data
free2<-free2%>%filter(is.na(X__1)==FALSE)
```

```{r}
free2<-free2%>%na.omit(dt)
```
