...
R can understand a web connection via the `url` command. Once that connection is established, we can download whatever we'd like. The 'url' command downloads the entire webpage.
...
```{r}
## url of pride and prejudice

prideprejudice = url("http://www.gutenberg.org/files/42671/42671-h/42671-h.htm")
# reading the content (first 15 lines)
prideprejudice_15 = readLines(prideprejudice, n = 150)
prideprejudice_15[1:150]
...
Store the URL as a variable, 'marathon_wiki'
The 'read_html' command will help us to pull the html table from that webpage, store it as 'marathon.'
```{r}
...
#Create a new variable, time2, converting time from hoursminutessecs(hms) to seconds
marathon<-marathon%>%
  mutate(Time2=hms(as.character(Time)))%>%
  mutate(Time2=period_to_seconds(Time2))

#Marathons are 42,200 meters long. We want to calculate speed in meters per second. The following equation calculates speed for each athlete.
...
We want to order the nationalities to see which nationalities ahve completed the fastest marathons. To do this we will mutate the nationality variable and reorder it by speed. We're going to create a new ggplot object, where the outcome is the speed of completion, the x variable is the date, and the color is the nationality of the athlete who completed the marathon.
```{r}
...
##Scrape the IAAF records table for men from the Wikipedia page on the world record for the mile, available here: https://en.wikipedia.org/wiki/Mile_run_world_record_progression. What's the median time? Enter it in the format MM:SS.t


```{r}
IAAF = "https://en.wikipedia.org/wiki/Mile_run_world_record_progression"

IAAF <-read_html(IAAF)%>%html_table(,fill=TRUE)

##professional males are the first table
IAAF_table<-tbl_df(data.frame(IAAF[[1]]))

median(IAAF_table$Time)
...
If you're aware of a resource with a large amount of data, look for an API first. If it does, then look for a "wrapper" package like the ACS package for R. If it doesn't ahve a wrapper package, then you'll need to write the request yourself.

If no API is available, then check robots.txt and terms of service for whether or not you can scrape. If it's ok, then create a program to scrape and structure data. All normal ethics requirements apply (IRB, etc).
...
Next, we'll combine the state and county fips into a single object. Use the paste command. 'paste0' prevents there from being a space between the two.
...
#easier names to work with, lowercase
names(county_to_zip)<-tolower(names(county_to_zip))

#Get the zip codes in selected county, Davidson county
county_to_zip<-county_to_zip%>%filter(state==state_fips,county==county_stub)%>%
  select(zcta5,state,county)

#store the list of zip codes of Davidson county as ziplist
ziplist<-county_to_zip$zcta5

#Get the zipcodes of every city in the USA. 
data(zip_codes)

##Filter the dataset, select just the zipcodes in our list. 
...
##geomake command specifies what geographies you'd like to work with. 
select_zip<-geo.make(zip.code=ziplist)

##the Fetch command allows us to interact with the API, ask for the data. The col.names is pretty to make it easy to read, and the verbose=T to tell what it's doing as it downloads this data
