...
x-axis: socioeconomic statis  y-axis: math scores
...
#Quick exercise
g3<-ggplot(data=els,
           aes(x=byincome,y=bynels2m)
           )

g3<-g3+geom_point(alpha=.5,size=.25) # Add points at x and y
g3
...
```{r}
#Quick exercise
g2a<-ggplot(data=filter(els,is.na(byincome)==FALSE),
            aes(x=byses1,y=bynels2m,
                color=as.factor(byincome) #notice the color option
                ))
## Let's make the dots smaller for readability
g2a<-g2a+geom_point(size=.1)

## Changing the Legend
g2a<-g2a+theme(legend.position="bottom"  , legend.title = 
               element_blank())

g2a<-g2a+ylab("Test Scores")+xlab("Socio Economic Status")
...
#overlay a line that is the predictions from mod_2
